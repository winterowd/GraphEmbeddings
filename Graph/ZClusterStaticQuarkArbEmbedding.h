#ifndef ZCLUSTERSTATICQUARKARBEMBEDDING
#define ZCLUSTERSTATICQUARKARBEMBEDDING
...
private:
    ...
    /// coefficient of each term  \prod_i \lambda^{n_i}_i (linear index) which is a polynomial in h_1, \bar{h}_1
    std::vector<GiNaC::ex> ZCoefficients; /// size \prod_i (N_i+1) (where N_i are the number of each type of bond for cluster)
...
private:
    ...
    std::pair<std::vector<UndirectedEdge>, std::array<int, ZClusterStaticQuarkArbEmbedding::NbrCouplings>> ZIntegrandToUndirectedEdgesAndBondCounts(const std::vector<bool>& permutation);
...
#endif // ZCLUSTERSTATICQUARKARBEMBEDDING
